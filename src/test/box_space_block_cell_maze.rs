...
fn test_construction() {
...

    {
        ...
        let maze = BoxSpaceBlockCellMazeCoordinator::<VecBuffer<BlockCellValue>, 2>::builder(coord_space).build();
    }
...

    {
        ...
        let maze = BoxSpaceBlockCellMazeCoordinator::<VecBuffer<BlockCellValue>, 2>::builder(coord_space).scale_factors_checked([2, 2]).build();
    }
    ...
    {
        let coord_space = BoxCoordinateSpace::new_checked([4, 4]);
        let maze = BoxSpaceBlockCellMazeCoordinator::<VecBuffer<BlockCellValue>, 2>::builder(coord_space).scale_factors_checked([7, 9]).build();
    }

    {
        let coord_space = BoxCoordinateSpace::new_checked([17, 61]);
        let maze = BoxSpaceBlockCellMazeCoordinator::<VecBuffer<BlockCellValue>, 2>::builder(coord_space).scale_factors_checked([12, 83]).build();
    }

    {
        let coord_space = BoxCoordinateSpace::new_checked([99, 99]);
        let maze = BoxSpaceBlockCellMazeCoordinator::<VecBuffer<BlockCellValue>, 2>::builder(coord_space).scale_factors_checked([11, 11]).build();
    }

    {
        let coord_space = BoxCoordinateSpace::new_checked([7, 7, 7]);
        let maze = BoxSpaceBlockCellMazeCoordinator::<VecBuffer<BlockCellValue>, 3>::builder(coord_space).scale_factors_checked([1, 1, 1]).build();
    }

    {
        let coord_space = BoxCoordinateSpace::new_checked([4, 8, 15, 16, 23]);
        let maze = BoxSpaceBlockCellMazeCoordinator::<VecBuffer<BlockCellValue>, 5>::builder(coord_space).scale_factors_checked([2, 2, 2, 2, 2]).build();
    }

    {
        let coord_space = BoxCoordinateSpace::new_checked([2, 2, 2, 2, 2, 2]);
        let maze = BoxSpaceBlockCellMazeCoordinator::<VecBuffer<BlockCellValue>, 6>::builder(coord_space).scale_factors_checked([3, 1, 4, 1, 5, 9]).build();
    }

    {
        let coord_space = BoxCoordinateSpace::new_checked([5, 5]);
        assert_that_code!(|| BoxSpaceBlockCellMazeCoordinator::<VecBuffer<BlockCellValue>, 2>::builder(coord_space).scale_factors_checked([0, 0]).build()).panics().with_having_message("All scale factors must be non-zero")
    }

    {
        let coord_space = BoxCoordinateSpace::new_checked([5, 5]);
        assert_that_code!(|| BoxSpaceBlockCellMazeCoordinator::<VecBuffer<BlockCellValue>, 2>::builder(coord_space).scale_factors_checked([1, 0]).build()).panics().with_having_message("All scale factors must be non-zero")
    }

    {
        let coord_space = BoxCoordinateSpace::new_checked([5, 5]);
        assert_that_code!(|| BoxSpaceBlockCellMazeCoordinator::<VecBuffer<BlockCellValue>, 2>::builder(coord_space).scale_factors_checked([0, 1]).build()).panics().with_having_message("All scale factors must be non-zero")
    }

    {
        let coord_space = BoxCoordinateSpace::new_checked([5, 5]);
        assert_that_code!(|| BoxSpaceBlockCellMazeCoordinator::<VecBuffer<BlockCellValue>, 2>::builder(coord_space).scale_factors_checked([1, usize::MAX]).build()).panics().with_having_message("The scaled dimensions do not all fit within a usize")
    }

    {
        let coord_space = BoxCoordinateSpace::new_checked([5, 5]);
        assert_that_code!(|| BoxSpaceBlockCellMazeCoordinator::<VecBuffer<BlockCellValue>, 2>::builder(coord_space).scale_factors_checked([usize::MAX, 1]).build()).panics().with_having_message("The scaled dimensions do not all fit within a usize")
    }

    {
        let coord_space = BoxCoordinateSpace::new_checked([5, 5]);
        assert_that_code!(|| BoxSpaceBlockCellMazeCoordinator::<VecBuffer<BlockCellValue>, 2>::builder(coord_space).scale_factors_checked([usize::MAX, usize::MAX]).build()).panics().with_having_message("The scaled dimensions do not all fit within a usize")
    }

    {
        let coord_space = BoxCoordinateSpace::new_checked([5, 5]);
        assert_that_code!(|| BoxSpaceBlockCellMazeCoordinator::<VecBuffer<BlockCellValue>, 2>::builder(coord_space).scale_factors_checked([1, 1]).padding([[usize::MAX, usize::MAX], [usize::MAX, usize::MAX]]).build()).panics().with_having_message("The full dimensions do not all fit within a usize")
    }

    {
        let coord_space = BoxCoordinateSpace::new_checked([5, 5]);
        assert_that_code!(|| BoxSpaceBlockCellMazeCoordinator::<VecBuffer<BlockCellValue>, 2>::builder(coord_space).scale_factors_checked([usize::MAX / 16, usize::MAX / 16]).build()).panics().with_having_message("The full dimensions specified are too large. The number of cells in the maze does not fit within a usize.")
    }
}
...
fn test_boundary_single_without_scaling_or_padding() {
    let coord_space = BoxCoordinateSpace::new_checked([5, 5]);
    let mut maze = BoxSpaceBlockCellMazeCoordinator::<VecBuffer<BlockCellValue>, 2>::builder(coord_space).scale_factors_checked([1, 1]).padding([[0, 0], [0, 0]]).build();
...
fn test_boundary_single_with_scaling_without_padding() {
    let coord_space = BoxCoordinateSpace::new_checked([5, 5]);
    let mut maze = BoxSpaceBlockCellMazeCoordinator::<VecBuffer<BlockCellValue>, 2>::builder(coord_space).scale_factors_checked([2, 2]).padding([[0, 0], [0, 0]]).build();
